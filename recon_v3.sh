...
idx=0
if [ "$NO_PREFLIGHT" -eq 1 ]; then
  ok "== Fase 1/2: preflight desativado (--no-preflight); todos os alvos seguem =="
else
  ok "== Fase 1/2: verificando quais alvos estão ativos (httpx) =="
fi
...
ok "== Fase 1/2 concluída: ${LIVE_COUNT} ativo(s), ${DEAD_COUNT} fora/ inválido(s) de ${TOTAL_TARGETS} =="
...
if [ "$LIVE_COUNT" -eq 0 ]; then
  ...
  ok "== Fase 2/2: executando módulos em ${LIVE_COUNT} alvo(s) ativo(s) =="
  idx=0
  for tgt in "${LIVE_TARGETS[@]}"; do
    idx=$((idx + 1))
    [ "$LIVE_COUNT" -gt 1 ] && log "===== [$idx/$LIVE_COUNT] $tgt ====="
    REQ_COUNT=0
    if setup_target "$tgt"; then
      dispatch_modes
      [ "$LIVE_COUNT" -gt 1 ] && ok "[$DOMAIN] Requisições: $REQ_COUNT"
    fi
    GRAND_TOTAL=$((GRAND_TOTAL + REQ_COUNT))
  done
fi
...
# Destaque final para o relatório consolidado de headers OWASP.
if [ "${want[techdeep]:-0}" -eq 1 ] && [ -s "$HEADERS_REPORT" ]; then
  ok "============================================================"
  ok "Relatório consolidado de headers OWASP: $HEADERS_REPORT"
  ok "  (aplicações + headers de segurança ausentes por alvo)"
  ok "============================================================"
fi
